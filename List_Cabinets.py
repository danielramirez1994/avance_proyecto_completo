...
class AplicacionGabinetes:
    ...
    def actualizar_espacio_restante(self, index_modificado):
        ...
        try:
            ...
            # Número de gavetas que no han sido modificadas
            num_no_modificadas = len(valores_gavetas) - num_modificados

            if num_no_modificadas > 0:
                nueva_altura = espacio_restante / num_no_modificadas

                # Asignar nuevas alturas a los campos que no han sido modificados
                for i, entry in enumerate(self.entries_altura_gavetas):
    ...
    def actualizar_campos_gavetas_editar1(self, event=None, num_gavetas=None, gabinete=None):
        ...
        try:
            ...
            self.label_espacio_restante.grid(row=num_gavetas, column=0, columnspan=2, padx=5, pady=5, sticky=tk.W)
    ...
    def actualizar_espacio_restante_editar(self, index_modificado):
        ...
        try:
            ...
            if total_modificado > self.espacio_disponible_editar:
                exceso = total_modificado - self.espacio_disponible_editar

                # Buscar la primera gaveta no modificada para restarle el exceso
                for i in range(len(valores_gavetas)):
                    if not self.valores_modificados_editar[i]:
                        nueva_altura = max(0, valores_gavetas[i] - exceso)  # Evitar negativos
                        self.entries_altura_gavetas_editar[i].delete(0, tk.END)
                        self.entries_altura_gavetas_editar[i].insert(0, f"{nueva_altura:.2f}")
                        break  # Solo ajustar una gaveta

                espacio_restante = 0  # Ya que se ajustó, el espacio restante es 0
